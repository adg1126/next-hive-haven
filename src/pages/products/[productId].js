	...
	return (
		<>
			...
			<main className='flex flex-col items-center text-dark w-full min-h-screen dark:text-light'>
				<section className='w-4/5 h-screen lg:h-full lg:flex-col flex justify-center py-24'>
					<div className='w-1/2 lg:w-full flex p-10 flex-col bg-white dark:bg-dark space-y-20 items-center top-10'>
						{productArr.map((p, i) =>
							p.href.slice(10) === productId ? (
								<Image
									...
									src={p.img}
									alt='productPic3'
									className='w-auto object-cover h-full border rounded-lg shadow-2xl lg:w-full md:inline-block md:w-full'
									priority
					...
					<div className='w-1/2 lg:w-full flex flex-col p-10 bg-white dark:bg-dark space-y-6 items-start top-10'>
						<AnimatedTextTypeWriter
							text='Beeswax Wrappers'
							className={`${raleway.className} lg:text-center font-black italic text-5xl xl:text-5xl lg:text-6xl md:text-4xl sm:text-3xl text-left`}
						/>
						{productArr.map((p, i) =>
							p.href.slice(10) === productId ? (
								<AnimatedTextTypeWriter
						...
						<p className='pr-10 lg:pr-0 font-medium w-full md:text-sm text-left lg:text-center'>
						...
						<div
							onClick={(e) => {}}
							className='flex flex-row lg:w-full whitespace-nowrap bg-blueGray-500 text-white font-bold text-xs px-3 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none ease-linear transition-all duration-150 bg-dark hover:bg-light hover:text-dark border-2 border-solid border-transparent hover:border-dark dark:bg-light dark:text-dark
